...
namespace DockerPanel.API.Endpoints
{
    ...
    public static class AcmeEndpoints
    {
        ...
        public static IEndpointRouteBuilder MapAcmeEndpoints(this IEndpointRouteBuilder app)
        {
            ...
            group.MapPost("providers/{provider}/test", TestProviderConnection).RequireWriteAccess();
            group.MapGet("accounts", GetAccounts);
            group.MapGet("accounts/{accountId}", GetAccount);
            group.MapPost("accounts", CreateAccount).RequireWriteAccess();
            group.MapDelete("accounts/{accountId}", DeleteAccount).RequireWriteAccess();
            group.MapPost("certificates/order", OrderCertificate).RequireWriteAccess();
            ...
            group.MapPost("certificates/orders/{orderId}/cancel", CancelCertificateOrder).RequireWriteAccess();
            group.MapPost("certificates/orders/{orderId}/challenges/{authorizationId}/complete", CompleteChallenge).RequireWriteAccess();
            group.MapGet("certificates/orders/{orderId}/challenges/{authorizationId}/status", CheckChallengeStatus);
            group.MapPost("certificates/orders/{orderId}/download", DownloadCertificateFromAcme).RequireWriteAccess();
            // ZIP 内含证书私钥（privkey.pem），仅 Admin 可下载
            group.MapGet("certificates/orders/{orderId}/download", DownloadCertificateZip).RequireAdmin();
            group.MapGet("certificates/{id}/download", DownloadCertificateById).RequireAdmin();
            group.MapGet("certificates/orders/{orderId}/challenges/pending", GetPendingChallenges);
            group.MapPost("certificates/{certificateId}/renew", RenewCertificate).RequireWriteAccess();
            group.MapPost("certificates/{certificateId}/auto-renewal/enable", EnableAutoRenewal).RequireWriteAccess();
            group.MapPost("certificates/{certificateId}/auto-renewal/disable", DisableAutoRenewal).RequireWriteAccess();
            group.MapPost("certificates/{certificateId}/retry", RetryCertificate).RequireWriteAccess();
            group.MapPost("certificates/{certificateId}/revoke", RevokeCertificate).RequireWriteAccess();
            group.MapDelete("certificates/{certificateId}", DeleteCertificate).RequireWriteAccess();
            group.MapGet("logs", GetOperationLogs);
            group.MapGet("certificates/{certificateId}/expiry", CheckCertificateExpiry);
            group.MapPost("certificates/auto-renew", AutoRenewCertificates).RequireWriteAccess();
            group.MapPost("certificates/fix-status", FixCertificateStatus).RequireWriteAccess();
            group.MapPost("domains/verify", VerifyDomainOwnership).RequireWriteAccess();
            group.MapPost("csr/generate", GenerateCsr).RequireWriteAccess();
            group.MapPost("certificates/validate", ValidateCertificate).RequireWriteAccess();
            group.MapGet("accounts/{accountId}/key", GetAccountKeyInfo);
            group.MapPost("keys/generate", GenerateKeyPair).RequireWriteAccess();
            // ACME 账户私钥导出，仅 Admin
            group.MapGet("accounts/{accountId}/key/export", ExportAccountKey).RequireAdmin();
            group.MapPost("keys/import", ImportAccountKey).RequireWriteAccess();
            ...
            group.MapPost("test/store-challenge", StoreTestChallenge).RequireWriteAccess();
            group.MapPost("certificates/renew-batch", RenewBatchCertificates).RequireWriteAccess();
